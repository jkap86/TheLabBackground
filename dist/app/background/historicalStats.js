...
const fetchWeekStats = async (season, week, season_type) => {
    console.log(`Fetching stats for ${season} ${season_type} - season, week ${week}.`);
    const graphqlQuery = {
        query: `
        query batch_scores {
            scores(
    ...
    const [schedule_week, projections_week, stats_week] = await Promise.all([
        axiosInstance.post("https://sleeper.com/graphql", graphqlQuery),
        axiosInstance.get(`https://api.sleeper.com/projections/nfl/${season}/${week}?season_type=${season_type}`),
        axiosInstance.get(`https://api.sleeper.com/stats/nfl/${season}/${week}?season_type=${season_type}`, {
            params: {
    ...
    const schedule_obj = {};
    schedule_week.data.data.scores.forEach((game) => {
    ...
    const player_ids = Array.from(new Set([
  ...
  `;
    const values = weekStats.flatMap((week_stat_obj) => {
        const { player_id, season, week, season_type, home, opp, kickoff, stats } = week_stat_obj;
    ...
    try {
        ...
        console.log(`${season} ${season_type} - season, week ${week} already in DB`);
    }
    console.log(`Values inserted for ${season} ${season_type} - season, week ${week}.`);
};

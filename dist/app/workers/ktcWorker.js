...
const syncAlltimeValues = async (type) => {
    ...
    for await (let sleeperId of sleeperIdsToUpdate.slice(0, increment)) {
        const syncPlayer = async (type) => {
            try {
                ...
                if (link) {
                    const response = await axiosInstance.get(`https://keeptradecut.com/${type}-rankings/players/` + link);
    ...
    if (Object.keys(ktc_dates).length === 0 &&
...
const matchPlayer = (player, allplayers, ktc_map) => {
    ...
    const matches = Object.keys(allplayers).filter((sleeper_id) => player.position.slice(0, 2)?.toLowerCase() ===
...
const queryKtcValues = async (type) => {
    const ktc_dates_db = await pool.query("SELECT * FROM common WHERE name = $1;", [`ktc_dates_${type}`]);
    const ktc_dates = ktc_dates_db.rows[0]?.data || {};
    const ktc_players_db = await pool.query("SELECT * FROM common WHERE name = $1;", [`ktc_players_${type}`]);
    const ktc_players = ktc_players_db.rows[0]?.data || {};
    const ktc_unmatched_db = await pool.query("SELECT * FROM common WHERE name = $1;", [`ktc_unmatched_${type}`]);
    ...
    fs.writeFileSync(`./app/utils/KtcSleeperIds_${type}.json`, JSON.stringify(ktc_map));
    return { ktc_dates, ktc_players, ktc_unmatched, ktc_map };
};
const queryAllPlayers = async () => {
    const allplayers_db = await pool.query("SELECT * FROM common WHERE name = $1;", ["allplayers"]);
    const allplayers = allplayers_db.rows[0]?.data || [];
    return {
        allplayers: Object.fromEntries(allplayers.map((player) => [
            player.player_id,
            player,
        ])),
    };
...
const addToKtcPlayers = async (type, data) => {
